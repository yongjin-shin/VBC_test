...
class BasicMAC_VBC:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False):
        ...
        msg_per_agent = ((-1 * msg_list) + msg_sum) / (self.n_agents - 1)
        ...
        #
        # agent_global_outputs = th.cat((agent0.view((8, 1, 14)), agent1.view((8, 1, 14)), agent2.view((8, 1, 14)),
        #                                agent3.view((8, 1, 14)), agent4.view((8, 1, 14)), agent5.view((8, 1, 14))), 1)
    ...
    def select_actions_comm_proto(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False):
        ...
        # dummy5 = self.env_blender(hidden_states[:, 5, :].view(self._parallel_batch_size, -1))
        #
        # num_eliminated_we = th.ones((self._parallel_batch_size, 6))
        ...
        # agent5 = (dummy0 + dummy1 + dummy2 + dummy3 + dummy4) / 5.0
        #
        # agent_global_outputs = th.cat((agent0.view((self._parallel_batch_size, 1, 14)),
        #                                agent1.view((self._parallel_batch_size, 1, 14)),
        #                                agent2.view((self._parallel_batch_size, 1, 14)),
        #                                agent3.view((self._parallel_batch_size, 1, 14)),
        #                                agent4.view((self._parallel_batch_size, 1, 14)),
        #                                agent5.view((self._parallel_batch_size, 1, 14))),
        #                               1)
        ...
        for i in range(0, self._parallel_batch_size):
            ...
            low_var_idx = (self.delta2 > q_std).nonzero()
            if len(low_var_idx):
                num_eliminated_we[i, low_var_idx] = 0
                msg_list[i, low_var_idx] = 0
            # if(th.std(dummy0[i,:], dim=0)<self.delta2):
            #     dummy0[i:,] = th.zeros(1,14)
            #     num_eliminated_we[i,0] = 0
            # if(th.std(dummy1[i,:], dim=0)<self.delta2):
